...
import cn.com.dtmobile.hadoop.biz.exception.mr.Lose4GReduce;

@SuppressWarnings("deprecation")
public class Lose4GJob extends Configured implements Tool {
	@Override
	public int run(String[] args) throws Exception {
		Configuration conf = getConf();
		String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
		if (otherArgs.length < 2) {
			System.err.println("Usage: Lose4GJob <in> <out>");
			System.exit(2);
		}
		...
		Job job = new Job(conf, "Lose4GJob");
		job.setJarByClass(Lose4GJob.class);
		job.setMapperClass(CommonMaps.class);
		job.setReducerClass(Lose4GReduce.class);
	...
	public static void main(String[] args) throws Exception {
		try {
			int returnCode = ToolRunner.run(new Lose4GJob(), args);
