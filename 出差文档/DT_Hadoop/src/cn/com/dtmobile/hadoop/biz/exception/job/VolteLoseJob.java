...
import cn.com.dtmobile.hadoop.biz.exception.mr.VolteLoseReduce;

@SuppressWarnings("deprecation")
public class VolteLoseJob extends Configured implements Tool {
	@Override
	public int run(String[] args) throws Exception {
		Configuration conf = getConf();
		String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
		if (otherArgs.length < 2) {
			System.err.println("Usage: VolteLoseJob <in> <out>");
			System.exit(2);
		}
		...
		Job job = new Job(conf, "VolteLoseJob");
		job.setJarByClass(VolteLoseJob.class);
		job.setMapperClass(CommonMaps.class);
		job.setReducerClass(VolteLoseReduce.class);
	...
	public static void main(String[] args) throws Exception {
		try {
			int returnCode = ToolRunner.run(new VolteLoseJob(), args);
