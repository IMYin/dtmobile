...
import cn.com.dtmobile.hadoop.biz.exception.mr.OffLine4GReduce;

@SuppressWarnings("deprecation")
public class OffLine4GJob extends Configured implements Tool {
	@Override
	public int run(String[] args) throws Exception {
		Configuration conf = getConf();
		String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
		if (otherArgs.length < 2) {
			System.err.println("Usage: OffLine4GJob <in> <out>");
			System.exit(2);
		}
		...
		Job job = new Job(conf, "OffLine4GJob");
		job.setJarByClass(OffLine4GJob.class);
		job.setMapperClass(CommonMaps.class);
		job.setReducerClass(OffLine4GReduce.class);
	...
	public static void main(String[] args) throws Exception {
		try {
			int returnCode = ToolRunner.run(new OffLine4GJob(), args);
